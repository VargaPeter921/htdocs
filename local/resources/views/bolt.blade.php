...
<table id="boltTermekek">
	<?php
		...
		{
			$ret = termekLekeres('', '', $keres, '','');
			$url = clean($ret[0])."-t".str_pad($ret[4], 6, "0", STR_PAD_LEFT);
			header("location: http://www.justabit.ebox-pc.hu/$url");
			...
			for ($x = 0; $x < ($maxMag); $x++)
			{
				...
				for ($i = 0; $i < ($maxSzel); $i++)
				{
					...
					if ($rand <= 25)
					{
						...
						echo '
							<td>
								<a href="/'.clean($ret[0]).'-t'.str_pad($ret[4], 6, '0', STR_PAD_LEFT).'">
								<div class="nagyBoltTermek">
									<div class="nagyTermekKep">
										<img class="hover" src="'.$ret[1].'" alt="">
									</div>
